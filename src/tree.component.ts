...
export class TreeComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output()
  public treeChange: EventEmitter<any> = new EventEmitter();
  ...
  private emitTree() {
    if (this.rootComponent && this.rootComponent['tree']) {
      const treeModel = this.buildModelFromTree(this.rootComponent['tree']);
      this.treeChange.emit(treeModel);
    }
  }

  private buildModelFromTree(tree) {
    const model: TreeTypes.TreeModel = {
      value: '',
      children: [],
