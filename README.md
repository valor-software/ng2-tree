...
- Events were reworked:
  - In V1 all events that were inherited from NodeDestructiveEvent used to have property `parent`. It is not the case anymore. If you need parent you should get it from `node` property in event object like `node.parent`;
  - All events used to have `node` property of type `TreeModel`. Now `node` property is of type [Tree](#tree-class) (as well as `node.parent`);
